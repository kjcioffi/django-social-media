...
class CreatePostView(TestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            "content": "".join(
                random.choice(string.ascii_letters) for _ in range(self.max_length)
            )
        }
    ...
    def test_post_created_successfully(self):
        ...
        self.assertEqual(
            {
                "profile_picture": self.profile.pic.url,
                "creator": response.wsgi_request.user.username,
    ...
    def test_exceeded_max_length(self):
        ...
        json = response.json()
        self.assertEqual(
            {"failure": "value too long for type character varying(30000)\n"}, json
        )
